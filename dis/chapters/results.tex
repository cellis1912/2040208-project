...
As shown in Table~\ref{tab:nasa_tlx_results}, participants reported consistently lower weighted \gls{NASATLX} scores when using the dashboard interface compared to the standard \gls{IDE}.
...
\begin{figure}[H]
    ...
    \caption{Two graphs displaying the differences in the \gls{NASATLX} scores for the two interfaces}\label{fig:Overallchart}
\end{figure}
...
\begin{table}[h]
\centering
\caption{Summary of \gls{NASATLX} Workload Scores (Mean $\pm$ SD)}
...
This visualisation illustrates the workload topology, where a larger surface area represents a higher total cognitive and emotional tax on the developer. 

\begin{figure}[H]
    \centering
    % Replace with your exported chart file
    \includegraphics[width=1\textwidth]{Charts/radar-chart.png} 
    \caption{Radar Chart: Visualising the reduction in multifaceted workload dimensions}
    \label{fig:radchart}
\end{figure}
...
\subsection{Efficiency and Time-on-Task Analysis (FR1)}
...
The \textit{Manage Panels} task provided the most striking difference in results. In the standard interface, several participants struggled to locate specific toggle 
settings, leading to a \textit{search-loop} behavior. Most of the participants tried to close each panel individually rather than using the \textit{Customise Layout} command.
...
The mean \gls{SUS} score of 92 places the dashboard within the “excellent” usability range, indicating that the interface imposes minimal learnability 
...
entirely. For the dashboard consistency in the design was key, all elements behaved predictably, all buttons were consistent. The results indicate that the Dashboard 
...
online, or pasting the error into an \gls{AI} chatbot. Notably, no participant described a systematic or confident approach to debugging, suggesting that 
...
feedback for aesthetic modifications, such as theme and font-size adjustments, where preferred to the baseline interface. This immediate confirmation allowed
 ...
 directly correlated with the higher frustration scores recorded in the \gls{NASATLX} and the higher time taken for these tasks.

Beyond visual configuration, non-visual features such as the \gls{AI} Task Breakdown and Code Error Checker were evaluated through a \textit{Think Aloud} protocol. Participants 
were asked to interact with these features and explain how they believed that the features would work as well as how they could interact with them. From this the results were consistently positive,
 all participants accurately identified the functional intent and operational flow of these complex tools.

 For the \gls{AI} Task Breakdown, a manual vs automated baseline was established. Before triggering the \gls{AI}, users were asked to verbally outline a high-level 
 decomposition of a programming task. While most provided basic, linear steps, the \gls{AI}-generated breakdown offered a more granular, comprehensive hierarchical 
 ...
 \gls{scaffolding} provided directly within the \gls{IDE} served as a significant aid in reducing \gls{executive_dysfunction}. By offloading the mental energy required for task 
...
``No File Open'' error, they responded to the Dashboard`s plain-language feedback significantly faster than they did to standard, often cryptic, \gls{IDE} diagnostic messages.
 ...
 technical hurdles do not escalate into focus-breaking frustrations. This transition from \textit{system-centric} to \textit{user-centric} feedback is a cornerstone of inclusive 
...
The Dashboard's design significantly reduced the number of interactions required to perform common tasks. Changing a theme requires just one click compared to an optimal four in the standard interface.
 ...
 familiar with this specific \gls{IDE}.Users unfamiliar with the \gls{IDE} often took a more circuitous route, increasing both click count and completion time beyond the baseline figures reported in Table~\ref{tab:time_savings}.
...
The Performance of NFR3 was evaluated by measuring the time elapsed between a user-initiated request and the rendering of the \gls{AI}-generated task breakdown in the Dashboard.

During the initial manual testing phase, I observed an average latency of approximately 3 seconds. While functional, this slightly exceeded the 2-second 
threshold defined in the project requirements.~\textcite{nielsen1994usability} established that a response time of under 2 seconds is generally perceived 
as instantaneous by users, which is critical for maintaining flow and minimising cognitive disruption.
 To achieve a more rigorous and repeatable measurement, I implemented an automated testing 
script that programmatically triggered the \textit{Task Architect}. This script recorded the time taken between request initiation and the 
appearance of the first response in the \gls{UI} window.


The automated tests revealed a refined average latency of 1.8 seconds, successfully meeting the performance requirement. Maintaining a 
response latency below two seconds is particularly critical for neurodivergent users, as prolonged system delays can trigger state recovery 
costs or disrupt attentional continuity, significantly increasing the likelihood of task abandonment.

\subsection{\gls{WCAG} 2.1 and Accessibility Compliance}
The Accessible Dashboard achieved a high score of 98/100 on the \gls{WCAG} 2.1 accessibility evaluation. Minor point deductions were exclusively 
related to the touch target sizing of specific interactive elements, which will be addressed in future iterations.  

This score was validated through a combination of:
\begin{itemize}
    \item \textbf{Automated Audits:} Utilising Lighthouse to identify structural and contrast-related issues.  
    \item \textbf{Manual Testing:} Verifying compatibility with assistive technologies to ensure a seamless experience for users with motor or visual impairments.  
\end{itemize}

The high level of compliance indicates that the tool meets stringent international standards, ensuring it is usable by a diverse range 
of developers. For a tool designed to support neurodivergent software engineers, this baseline of accessibility is essential, as it ensures 
that the interface itself does not introduce a secondary executive tax or sensory barriers that could negate the benefits of the \gls{AI}-driven scaffolding.
...
The 82\% reduction in weighted workload suggests that the executive tax paid by \gls{neurodivergent} developers
 is not an inherent trait of their neurology, but a byproduct of \gls{neurotypical-bias} in tool design. By suppressing \gls{EL} 
 (via the Minimalist Toggle) and \gls{scaffolding} \gls{IL} (via the Task Architect), the system maximises \gls{GL}, allowing 
 cognitive resources to be redirected toward core algorithmic problem-solving. 

Beyond the primary target demographic, the results provide strong evidence for the curb-cut principle within the \gls{IDE} ecosystem:
\begin{itemize}
    \item Validation of Universal Utility: The Excellent \gls{SUS} score of 92 suggests the interface is perceived not as a niche medical tool, but as a high-performance 
productivity suite suitable for any developer.
    \item Universal Operational Efficiency: While designed to mitigate executive dysfunction, the 89.49\% time reduction in panel management (Table~\ref{tab:time_savings}) 
    represents a universal efficiency gain. This suggests that the nested menus in a standard \gls{IDE} is a friction point for all users.
    \item Support for Deep Work: \gls{EL} is a cognitive burden for every developer. By providing a single-click path to a minimalist environment, 
the plugin supports the deep work and flow-state requirements of neurotypical developers just as effectively as it supports task initiation for those with \gls{ADHD} or \gls{ASD}.
    \item Reduced Verification Anxiety: The implementation of instantaneous feedback for environment changes (Section 4.7) eliminates the verification anxiety common in 
complex settings menus, streamlining the workflow for the general developer population. 
\end{itemize}
...
\begin{itemize}
    ...
    The study utilised a limited cohort ($N=5$). Furthermore, participants were known to the researcher 
    ...
    Due to stringent ethical considerations regarding the storage of sensitive personal data and the project's
     ...
     definitively categorised by specific clinical diagnoses.
...
Overall, while the results are promising and suggest significant benefits, they should be interpreted with caution due to the aforementioned limitations.
It would be essential to conduct larger-scale studies with more diverse and clinically verified participant pools to validate these findings and ensure they generalise 
across the broader population of developers.
