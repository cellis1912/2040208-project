...
Having a single switch that closes all UI elements will directly targets the suppression of \gls{EL} to free up mental bandwidth instantly without having to navigate menus.
While the AI task decomposition tool acts as a cognitive prosthetic that scaffolds the \gls{IL} of project planning, preventing \gls{task_paralysis}.
...
While Sweller’s Cognitive Load Theory focuses primarily on the nature and management of cognitive load, Mayer’s \gls{CTML} 
...
\begin{itemize}
    \item Inhibitory Control: The ability to ignore distractions. For ADHD developers, the \textit{pop-up} notifications of a linter can break a fragile state of focus.
    \item \Gls{working_memory}: Dyslexic developers often have a high capacity for 3D spatial reasoning but a limited verbal \gls{working_memory}, making syntax-heavy languages like C++ more taxing than cleaner languages like Python.
...
Furthermore, while \gls{hyperfocus} is frequently cited as a \textit{superpower} for developers, it can lead to severe mental and physical exhaustion if left unregulated. The structured 
...
While high contrast is vital for clarity, pure high contrast (e.g., stark white text on a pure black background) can be counterproductive for those with \gls{ASD} or specific visual stresses.
 ...
 To address this, the most effective models for \gls{neurodiversity} are often \textit{Soft High Contrast} themes. These utilize high-visibility colors against deep, non-pure-black backgrounds 
...
model to decompose a broad, abstract goal into a granular, step-by-step plan. This process mitigates the \textit{activation energy} barrier identified in \gls{ADHD} populations. By prompting a model to 
