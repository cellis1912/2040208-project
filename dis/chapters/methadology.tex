...
I first designed the \gls{UI} in figma, which allowed me get a clear vision of how the plugin would look and function. 
This also guaranteed that the design of the plugin was in line with the requirements that I had set out in the previous section.
Before having an idea for the UI I was already aware that I would implement the UI using HTML and CSS in a Webview, which allowed me to create a custom dashboard 
...
\subsubsection{Task Architect - AI-Driven \Gls{scaffolding} (FR2)}
...
the necessary context for the feature to function, reducing confusion and promoting a sense of control.
    \item Extraction: Using the vscode.languages.getDiagnostics API, the module identifies active compiler issues. To minimize noise, 
    ...
    This list is then dispatched to the resident gpt-4o model via the VS Code Language Model API.
\end{itemize}

\textbf{Prompt Engineering for Direct Instruction}


To satisfy the requirement for beginner-friendly, non-technical feedback, the module employs a strictly constrained system prompt. The AI is instructed to:
\begin{itemize}
