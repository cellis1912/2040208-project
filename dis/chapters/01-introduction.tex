...
romanticised as a \textit{superpower} due to the high capacity for deep logical analysis
...
these Executive Functioning~(\gls{EF}) barriers. This research seeks to transition the \gls{IDE} from a passive tool into an active cognitive partner through \gls{AI}-driven support.
...
multi-step planning. For developers with \gls{ADHD}, \gls{ASD}, or \gls{dyslexia}, these tools often exacerbate \gls{extraneous_cognitive_load}, 
...
frequently experience \gls{task_paralysis}, difficulty initiating tasks during a project~\cite{bramham2009executive}, while those with \gls{ASD} may struggle with the \gls{double_empathy}, a disconnect between the communication
styles of neurodivergent and neurotypical individuals, leading to them misinterpreting 
ambiguous or implied requirements from team members~\cite{Milton01102012}. Existing \gls{AI} tools focus almost exclusively on code generation rather than process support. Consequently, there is a critical need 
for an \gls{IDE} intervention that acts as a digital ramp, filtering noise and providing the \gls{scaffolding} necessary to bypass the executive freeze response.
...
Before going into the specific technical barriers of modern \glspl{IDE}, it is necessary to define \gls{neurodiversity} and the cognitive
 frameworks that underpin this research. Originally coined by Judy Singer~\cite{singer1999why} in the late 1990s, \gls{neurodiversity} views neurological differences, such 
 as \gls{ASD}, \gls{ADHD}, and \gls{dyslexia}, as natural variations in the human genome rather than purely medical deficits.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{l >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
        ...
        \addlinespace
        \gls{ADHD} & Creativity, \gls{hyperfocus} on passions, adaptability, high energy, ability to multitask or rapidly switch contexts. & Distractibility in chaotic settings, struggles with time management and organisation leading to \gls{time-blindness} and issues with \gls{task_initiation}, impulsivity in structured environments.\\
        \addlinespace
...
The primary objective of this project is to design, implement, and evaluate an assistive VS Code plug-in that reduces extraneous cognitive load and mitigates the effects of
 \gls{executive_dysfunction} for \gls{neurodivergent} software developers. By introducing a cognitive prosthetic layer, the project seeks
  to determine if targeted \gls{UI} simplification and \gls{AI}-driven task \gls{scaffolding} can measurably improve the developer experience.

\subsubsection{Research Objectives}

\textbf{Objective 1:} Interface Optimisation (The Minimalist Toggle)

Target: Develop a single \textit{Minimalist Toggle} feature that simultaneously hides non-essential User Interface~(\gls{UI}) elements, including the Activity Bar, Status Bar, Mini-map, and Breadcrumbs.
...
Justification: This directly addresses the issue of visual overload by reducing the number of stimuli competing for the developer's attention, thus lowering the Extraneous
 Cognitive Load and creating a more focused coding environment \cite{sweller1988cognitive}, reducing visual noise that leads to distraction and sensory overwhelm for developers with \gls{ASD} and \gls{ADHD}.

\textbf{Objective 2: }\gls{AI}-Driven Task Decomposition (The Task Architect)

Target: To implement an \gls{LLM}-integrated module that translates ambiguous programming goals into structured, editable checklists to allow the user to have final control over the task breakdown. This should be
implemented via the Open\gls{AI} Application Programming Interface~(\gls{API}) that breaks down high-level requirements~\cite{ringel2019ai}.


Justification: This supports \gls{task_initiation} and reduces the energy required to overcome \gls{task_paralysis}, a primary barrier for developers with \gls{ADHD} and \gls{ASD}~\cite{gama2025socio}.
...
Justification: This minimises the cognitive demand of interpreting dense text, preventing rabbit holes and supporting the literal communication needs identified in the \Gls{double_empathy}
 problem. By simplifying the feedback loop, developers can maintain focus on the core problem rather than getting lost in peripheral details. This is particularly beneficial for those with \gls{ASD},
  who may struggle with implied logic and prefer clear, direct instructions. Additionally, it can help mitigate the \gls{hyperfocus} trap by providing concise guidance that keeps the developer on track 
  without overwhelming them with information.

\textbf{Objective 4: }Visual Accessibility Customisation (The Visual Styler)

Target: To create at least two visual presets, such as \textit{Soft High Contrast}, that adhere to Web Content Accessibility Guidelines~(\gls{WCAG} 2.1), set of international standards designed to make web content more accessible to people with disabilities it can also help with visual processing, eye strain and enhance readability~\cite{bengtsson2025web}.
...
Justification: This provides a standardised scientific metric to confirm if the tool successfully reduces mental demand and frustration for the target demographic.

\subsection{Dissertation Structure}

This dissertation is organised into five sections, following a logical progression from theoretical grounding to empirical validation:
\begin{itemize}
  ...
  software engineering as well as the shift toward a more \gls{social_model} rather than a \gls{medical_model}.
  \item \textbf{Section 3: Design and Implementation} Details the user requirements and the three-layer system architecture. This chapter explains the technical
   execution of \textit{Minimalist Toggle} and \textit{Task Architect} as well as the visual presets and styling tools. I will also discuss issues encountered during development and how they were resolved, such as the design of the UI and issues with the \gls{AI} integration and prompt generation.
  \item \textbf{Section 4: Results and Discussion} Presents the quantitative and qualitative findings from the user study ($N=5$). It analyses the 82\% reduction in perceived workload measured by the \gls{NASATLX} 
